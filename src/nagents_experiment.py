...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_n', type=int, default=10000, help='Number of training matrices')
    ...
    for N in sizes_to_test:
        ...
        sensingmasks = SensingMasks(train_data, args.r, N, args.density)
        ...
        print(f"Training time for {N} agents: {(end - start).total_seconds() / 60:.4f} minutes.")
        ...
        print(f"Test Set Performance for {N} agents: | ",
    ...
    logsizes = np.log2(sizes_to_test)
    axs[0].plot(logsizes, np.log(results_train_loss), label="Train Loss", marker="o")
    axs[0].plot(logsizes, np.log(results_test_loss), label="Test Loss", marker="o")
    axs[1].plot(logsizes, results_train_accuracy, label="Train Accuracy", marker="o")
    axs[1].plot(logsizes, results_test_accuracy, label="Test Accuracy", marker="o")
    axs[2].plot(logsizes, results_train_agreement, label="Train Agreement", marker="o", linestyle='dotted')
    axs[2].plot(logsizes, results_test_agreement, label="Test Agreement", marker="o", linestyle='dotted')
    ...
    for ax in axs:
        ax.set_xlabel("Log2 nb of agents")
        ax.legend()
        ax.grid(True)
    fig.suptitle(f"Training performance after {args.epochs} epochs, by nb of agents (fixed 4-lattice)")
    txt = f"Hyperparameters: " \
        f"T = {args.t}, m = {args.m}, r = {args.r}, density = {args.density}, " \
        f"hidden_dim = {args.hidden_dim}, att_heads = {args.att_heads}\n" \
        f"dropout = {args.dropout}, init_lr = {args.lr}, batch_size = {args.batch_size}, " \
        f"steps = {args.steps}, train_n = {args.train_n}, test_n = {args.test_n}, nres = {args.nres}."
    plt.figtext(0.5, 0.0, txt, ha='center', va ='top', fontsize=9)
    fig.subplots_adjust(bottom=0.3)
    fig.tight_layout()
    
    fig.savefig("results/nagents_experiment.png", bbox_inches="tight")
    plt.close(fig)
