...
if __name__ == '__main__':
    ...
    for dim in sizes_to_test:
        torch._dynamo.reset()
        model = DistributedDotGAT(
            device=device, input_dim=args.t * args.m, hidden_dim=dim, n=args.t, m=args.m,
        ...
        aggregator = CollectiveClassifier(
            num_agents=args.num_agents, agent_outputs_dim=dim, m = args.m
        ).to(device)
        ...
        print(f"Training time for dim {dim}: {(end - start).total_seconds() / 60:.4f} minutes.")
        ...
        print(f"Test Set Performance for dim {dim}: | ",
    ...
    fig.savefig("results/hdim_experiment.png", bbox_inches="tight")
    plt.close(fig)
